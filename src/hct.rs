...
impl ViewingConditions {
    ...
    pub fn srgb() -> Self {
        ...
        // nbb - chromatic induction factor
...
fn argb_from_lstar(lstar: f64) -> u32 {
    ...
    let component = delinearize(y / 100.0);
...
mod tests {
    ...
    fn test_hct_from_argb_red() {
        ...
        assert!(hct.chroma() > 10.0, "Red chroma: {}", hct.chroma());
    }
    ...
    #[test]
    fn test_hct_roundtrip_is_reasonably_stable() {
        ...
        assert!(r_diff <= 25, "Red diff too large: {r_diff}");
        assert!(g_diff <= 25, "Green diff too large: {g_diff}");
        assert!(b_diff <= 25, "Blue diff too large: {b_diff}");
    }
